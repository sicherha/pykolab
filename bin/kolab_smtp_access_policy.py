...
class PolicyRequest(object):
            ...
            if not key == 'recipient':
                ...
                if not policy_request['recipient'].strip() == '':
                    self.recipients = list(
                        set(self.recipients + [policy_request['recipient']])
                    )
    ...
    def add_request(self, policy_request={}):
        ...
        # Normalize email addresses (they may contain recipient delimiters)
        if 'recipient' in policy_request:
            policy_request['recipient'] = normalize_address(policy_request['recipient'])
    ...
    def parse_ldap_dn(self, dn):
        ...
        if len(ldap_dn) > 0:
            search_attrs = conf.get_list('kolab_smtp_access_policy', 'address_search_attrs')
    ...
    def parse_ldap_uri(self, uri):
        ...
        (_protocol, _server, _port, _base_dn, _attrs, _scope, _filter) = parsed_uri
    ...
    def parse_policy(self, _subject, _object, policy):
        ...
        for rule in policy:
            ...
            if ldap_dn is not None and len(ldap_dn) > 0:
                if _prefix == '-':
                ...
                if ldap_uri is not None and len(ldap_uri) > 0:
                    if _prefix == '-':
    ...
    def verify_alias(self):
        ...
        for search_attr in search_attrs:
            if search_attr not in self.sasl_user:
        ...
        for search_attr in search_attrs:
            if search_attr in self.sasl_user:
    ...
    def verify_authenticity(self):
        ...
        if self.sasl_domain is None:
            if len(self.sasl_username.split('@')) > 1:
                ...
                sasl_username = "%s@%s" % (
                    self.sasl_username, self.sasl_domain
                )
        ...
        sasl_users = self.auth.find_recipient(sasl_username, domain=self.sasl_domain)
    ...
    def verify_delegate(self):
        ...
        if 'kolabdelegate' not in self.sender_user:
            # Got a final answer here, do the caching thing.
        ...
        elif self.sender_user['kolabdelegate'] is None:
    ...
    def verify_recipient(self, recipient):
        ...
        if not self.sasl_username == '' and self.sasl_username is not None:
            log.debug(
                _("Verifying authenticated sender '%(sender)s' with sasl_username '%(sasl_username)s' for recipient '%(recipient)s'") % (
                    self.__dict__
                ),
                level=8
            )
        ...
        if verify_domain(sasl_domain):
            if self.auth.domains is not None and sasl_domain in self.auth.domains:
                log.debug(
                ...
                log.debug(
                    _("Domain %s is a primary domain") % (sasl_domain),
                    level=8
                )

        else:
            log.warning(
                _("Checking the recipient for domain %s that is not ours.") % (sasl_domain)
            )
            ...
            if records is not None and len(records) == len(self.recipients):
                ...
                for record in records:
                    ...
                    if not recipient_found:
                        reject(
                            _("Sender %s is not allowed to send to recipient %s") % (
                                self.sender,
                                recipient
                            )
                        )
    ...
    def verify_sender(self):
        ...
        if cache is not False:
            ...
            if records is not None and len(records) == len(self.recipients):
                log.info(
                    _("Reproducing verify_sender(%r) from cache") % (self.__dict__)
                )
        ...
        if self.verify_authenticity() is False:
        ...
        if self.sasl_user_is_delegate is False:
            if self.sasl_user_uses_alias is False:
                if not conf.allow_unauthenticated:
                    reject(_("Sender uses unauthorized envelope sender address"))

        if self.sasl_user_is_delegate:
        ...
        elif self.sasl_user is not None:
        ...
        if 'kolaballowsmtprecipient' in recipient_policy_user:
        ...
        # Otherwise,parse the policy obtained.
            ...
            for recipient in self.recipients:
                ...
                if not recipient_allowed:
                    reject(
                        _("Sender %s not allowed to send to recipient %s") % (
                            recipient_policy_user['dn'],
                            recipient
                        )
                    )
...
def cache_cleanup():
    ...
    session.query(PolicyResult).filter(
...
def permit(message, policy_request=None):
    ...
    # Same here.
    if policy_request.sasl_username is None:
        print("action=PERMIT\n\n")
        sys.exit(0)
    ...
    if policy_request.sasl_user_is_delegate:
        ...
        # Global setting?
        if delegate_sender_header is None:
    ...
    # and/or X-Sender headers.
        ...
        # Global setting?
        if alias_sender_header is None:
    ...
    if sender_header or xsender_header:
        ...
        if xsender_header and xheader is not None:
...
def verify_domain(domain):
    ...
    if mydomains is not None and domain in mydomains:
...
if __name__ == "__main__":
    ...
    access_policy_group.add_option(
        ...
        action="store_true",
        default=False,
        help=_("Drop the caching tables from the database and exit.")
    )
